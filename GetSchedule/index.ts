...
function calculateNextStage(startTime: number, seed: TimeZone): TimeZone {
  ...
  next.stages[4] = seed.stages[4].map(mapZone);
  next.stages[5] = seed.stages[5].map(mapZone);
  next.stages[6] = seed.stages[6].map(mapZone);
  next.stages[7] = seed.stages[7].map(mapZone);
